...
@Singleton
class leaguesChopperOverlay extends OverlayPanel
{
	private final Client client;
	private final leaguesChopperPlugin plugin;
	private final leaguesChopperConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private leaguesChopperOverlay(final Client client, final leaguesChopperPlugin plugin, final leaguesChopperConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Leagues - Chopper"));
	}

	@Override
	public Dimension render(Graphics2D graphics)
	{
		if (plugin.botTimer == null || !plugin.startChopper || !config.enableUI())
		{
			return null;
		}
