...
@Singleton
class leaguesPlankerOverlay extends OverlayPanel
{
	private final Client client;
	private final leaguesPlankerPlugin plugin;
	private final leaguesPlankerConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private leaguesPlankerOverlay(final Client client, final leaguesPlankerPlugin plugin, final leaguesPlankerConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Leagues - Planker"));
	}

	@Override
	public Dimension render(Graphics2D graphics)
	{
		if (plugin.botTimer == null || !plugin.startPlanker || !config.enableUI())
		{
			return null;
		}
